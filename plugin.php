...
function outvoice_enqueue_assets() {
    ...
    $options = get_option('outvoice_plugin_options');
    $data['apiKey'] = esc_attr($options['api_key']);
    wp_localize_script( 'outvoice-sidebar', 'outvoiceData', $data );
    
    wp_register_style( 'outvoice-react-css', plugin_dir_url( __FILE__ ) . 'css/outvoice-react.css', false, '1.0.3' );
...
function outvoice_mce_before_init() {
  if ('post' === get_post_type() && is_plugin_active( 'classic-editor/classic-editor.php' ) ) {
    wp_enqueue_script( 'outvoice', plugin_dir_url( __FILE__ ) . 'js/outvoice.js', array( 'ov-combobox' ), '1.1.1', false );
    // check login status.
    $status = 1;
    $token  = outvoice_token_check();
    if ( ! $token ) {
...
/**
 * Save tokens to DB in wp_options table
 *
...
function outvoice_save_tokens( $tokens ) {
    ...
    // write tokens to options table.
    $new_tokens = array(
        'access'  => $tokens['access_token'],
        'refresh' => $tokens['refresh_token'],
        'expires' => time() + 3000,
    );
    $users = (array) get_option( 'outvoice_users', array() );
    $users[$id] = $new_tokens;
    update_option('outvoice_users', $users, null);
    return 'saved';
}
...
function outvoice_token_check() {
	...
	$ov     = new Outvoice\Api\Outvoiceapi( $tokens );
...
function outvoice_admin_script( $hook ) {
	...
	if ( is_plugin_active( 'classic-editor/classic-editor.php' ) ) {
        ...
        wp_register_style('outvoice_css', plugin_dir_url(__FILE__) . 'css/outvoice.css', false, '1.0.16');
	...
	if ( 'publish' === $new_status || 'future' === $new_status ) {
		if ( ! empty( $_POST['outvoice-freelancer'] ) && ! empty( $_POST['outvoice-amount'] ) && ! empty( $_POST['outvoice-currency'] ) ) {
			...
			if ( isset( $_POST['outvoice_post_nonce'] ) && wp_verify_nonce( sanitize_key( $_POST['outvoice_post_nonce'] ), 'outvoice_post' ) ) {
                ...
                $tokens = outvoice_retrieve_tokens();
				$transaction = new Outvoice\Api\Outvoiceapi( $tokens );
				...
				// add date if future scheduled
                if ( isset( $_POST['post_date'] ) ) {
                    $info['scheduled'] = sanitize_text_field( wp_unslash( $_POST['post_date'] ) );
                }

                $options = get_option('outvoice_plugin_options');
				$info['apiKey'] = esc_attr($options['api_key']);
...
function outvoice_failure_notice() {
	...
	<div class="notice notice-error is-dismissible">
		<p><?php print 'OutVoice detected a potential problem. ' . wp_kses( get_transient( 'outvoice_error' ), array() ); ?></p>
	</div>
		<?php
...
function outvoice_list_freelancers() {
	...
	$list = new Outvoice\Api\Outvoiceapi( $options );
        ...
        if ( isset( $_POST['ov-submit'] ) ) {
            ...
            if ( 'Log In' === $_POST['ov-submit'] ) {
                if ( isset( $_POST['mail'] ) && isset( $_POST['pass'] ) ) {
                    ...
                    if ( $ov->generate_tokens( $mail, $pass ) ) {
                        $tokens = [
                            'access_token' => $ov->access_token,
                            'refresh_token' => $ov->refresh_token,
                        ];
...
function outvoice_add_settings_page()
{
    add_options_page('OutVoice Configuration', 'OutVoice', 'manage_options', 'outvoice-settings', 'outvoice_render_plugin_settings_page');
}

add_action('admin_menu', 'outvoice_add_settings_page');

function outvoice_render_plugin_settings_page() {
    ?>
    <div class="wrap">
    <h1>OutVoice Settings</h1>
    <form action="options.php" method="post">
        <?php
        settings_fields( 'outvoice_plugin_options' );
        do_settings_sections( 'outvoice_plugin' ); ?>
...
function outvoice_register_settings()
{
    register_setting('outvoice_plugin_options', 'outvoice_plugin_options', 'outvoice_plugin_options_validate');
    add_settings_section('api_settings', '', 'outvoice_plugin_section_text', 'outvoice_plugin');

    add_settings_field('outvoice_plugin_setting_api_key', 'API Key', 'outvoice_plugin_setting_api_key', 'outvoice_plugin', 'api_settings');
}

add_action('admin_init', 'outvoice_register_settings');

function outvoice_plugin_section_text()
{
    echo '<p>If you do not know your API Key, contact <a href="mailto:support@outvoice.com">support@outvoice.com</a></p>';
}

function outvoice_plugin_setting_api_key()
{
    $options = get_option('outvoice_plugin_options');
    echo "<input id='outvoice_plugin_setting_api_key' name='outvoice_plugin_options[api_key]' type='text' size='40' value='" . esc_attr($options['api_key']) . "' />";
}
