...
class Outvoiceapi {
	 ...
	 * @var string
	 */
	protected $module_version = 'wordpress-2.0.2';
    ...
    public function __construct( $tokens = NULL ) {
        if ( isset( $tokens['access'] ) && isset( $tokens['refresh'] ) && isset( $tokens['expires'] ) ) {
            $this->set_tokens($tokens['access'], $tokens['refresh']);
    ...
    public function save_tokens() {
        ...
        $updated = update_option('outvoice_users', $users, null);
        return TRUE;
    }
	...
	public function auth_check() {
		...
		$output['expires']       = $this->token_expires;
		return $output;
	}
